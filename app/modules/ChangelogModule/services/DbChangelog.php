...
class DbChangelog extends \Nette\Object {
	...
	private $changelogDir = '/changelog/';
	...
	public function __construct(\Nette\Database\Connection $connection, \Nette\Database\Table\Selection $changelogTable, $appDir)
	{
		...
		$this->changelogPath = $appDir.$this->changelogDir;
		if (!is_dir($this->changelogPath)) {
			throw new \Nette\DirectoryNotFoundException('Dir "'.$this->changelogPath.' not found in "'.$appDir.'"');
		}
		if (!is_writeable($this->changelogPath)) {
			throw new \Exception('Dir "'.$this->changelogPath.' in "'.$appDir.'" is not writeable.');
		}
	}
	...
	public function addNewQueries($description, $queries)
	{
		...
		foreach ($queries as $query) {
			...
			$data = array(
				...
				'ins_timestamp' => $time,
				'ins_dt' => new \DateTime,
				'ins_process_id' => 'DbChangelog::addNewQueries'
			);
		...
		foreach (Finder::findFiles('*.sql')->in($this->changelogPath) as $key => $file) {
			...
			foreach ($queries as $query) {
				...
				$data = array(
					...
					'ins_dt' => new \DateTime,
					'ins_process_id' => 'DbChangelog::areNewChangelogData'
				);
				$this->changelogTable->insert($data);
			}
