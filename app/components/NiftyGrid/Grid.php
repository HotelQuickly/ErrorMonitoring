...
abstract class Grid extends \Nette\Application\UI\Control
{
		...
		try{
			...
			foreach($this->filter as $name => $value){
				if(!$this->columnExists($name)){
					throw new UnknownColumnException("Uknown column $name");

				}
				if(!$this['columns-'.$name]->hasFilter()){
					throw new UnknownFilterException("Unknow filter for column $name");
				}
	...
	protected function createComponentGridForm()
	{
		...
		$form[$this->name]['perPage']->addSelect("perPage","Items on page:", $this->perPageValues)
			...
			->addData("link", $this->link("changePerPage!", array("perPage" => null)));
